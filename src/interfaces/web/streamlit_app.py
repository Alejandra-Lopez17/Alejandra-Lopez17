...
from domain.services.anomaly_detection import IsolationForestDetector, create_anomaly_detector
from domain.services.quality_classifier import QualityClassifier, QualityLabel, create_quality_classifier
from domain.services.clustering import DocumentClusterer, VisualizationService, ClusterAnalysis
from infrastructure.persistence.chroma_repository import ChromaDocumentRepository
from infrastructure.file_handlers import MarkdownHandler, RSTHandler, PDFHandler, TextHandler
...
class DocumentExplorerApp:
    ...
    def _initialize_services(self) -> None:
        ...
        with st.spinner("Loading models..."):
            try:
                ...
                self.anomaly_detector = create_anomaly_detector("isolation_forest", contamination=0.1)
            ...
            if results["ids"] and len(results["ids"]) > 0:
                for i in range(len(results["ids"])):
                    doc = TechnicalDocument(
                        ...
                        document_type=DocumentType(results["metadatas"][i].get("type", "markdown")),
                    )
    ...
    def run(self) -> None:
        ...
        st.markdown("Explore your document corpus with interactive visualization and quality analysis")
                        ...
                        for uploaded_file in uploaded_files:
                            ...
                            if handler_class:
                                ...
                                if doc and self.embedding_service and self.repo:
                                    embedding = self.embedding_service.generate_document_embedding(doc)
        ...
        if len(documents) == 0:
            ...
            st.code("python -m src.interfaces.cli.main process data/documents", language="bash")
            return
    ...
    def _render_cluster_visualization(
        ...
        with col2:
            ...
            n_clusters = st.slider("Number of Clusters", 2, max_clusters, min(5, n_docs))
            ...
            # Reducir dimensiones
            coords_2d = self.visualizer.reduce_dimensions(embeddings, n_components=2, method=method)
    ...
    def _render_quality_analysis(
        ...
        if hasattr(self.quality_classifier, '_is_fitted') and self.quality_classifier._is_fitted:
    ...
    def _render_anomaly_detection(
        ...
        with col1:
            ...
            contamination = st.slider(
                "Expected contamination (anomaly proportion)",
                0.01, 0.5, 0.1
            )
    ...
    def _render_semantic_search(
        ...
        query = st.text_input("Enter your query:", placeholder="Example: how to configure API...")
...
def main():
    ...
    st.sidebar.markdown('<div class="student-name">ALEJANDRA LOPEZ</div>', unsafe_allow_html=True)
    st.sidebar.markdown('<div class="student-role">Software Engineer</div>', unsafe_allow_html=True)
    ...
    st.sidebar.markdown('<div class="project-title">Data Science and Machine Learning Vault</div>', unsafe_allow_html=True)
